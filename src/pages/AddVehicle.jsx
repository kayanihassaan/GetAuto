...
export default function AddVehicle({ theme }) {
  const navigate = useNavigate()
  const { user } = useAuth()
  const [form, setForm] = useState({
    type: 'car', name: '', variant: '', rentPerDay: '',
  ...
  return (
    <div className={`${bg} min-h-screen`}>
      <div className="max-w-lg mx-auto px-4 py-6">
        <h1 className={`text-2xl font-bold ${textColor} mb-6`}>Add a Vehicle</h1>

        {success ? (
          <div className={`${cardBg} rounded-xl p-8 text-center shadow-md`}>
            <p className="text-4xl mb-3">✅</p>
            <p className={`text-lg font-semibold ${textColor}`}>Vehicle listed successfully!</p>
            <p className={muted}>Redirecting to your vehicles...</p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className={`${cardBg} rounded-xl p-6 shadow-md space-y-4`}>
            ...
            <div>
              ...
              <input name="name" value={form.name} onChange={handleChange} required className={inputStyle} placeholder="e.g. Toyota Corolla" />
            </div>
            <div>
              <label className={`block text-sm font-medium ${textColor} mb-1`}>Variant/Model *</label>
              <input name="variant" value={form.variant} onChange={handleChange} required className={inputStyle} placeholder="e.g. XLI 2024" />
            </div>
            <div>
            ...
            <div>
              ...
              <input name="location" value={form.location} onChange={handleChange} className={inputStyle} placeholder="e.g. Lahore" />
            </div>
            <div>
              <label className={`block text-sm font-medium ${textColor} mb-1`}>Description</label>
              <textarea name="description" rows="3" value={form.description} onChange={handleChange} className={inputStyle} placeholder="Describe your vehicle..." />
            </div>
            <div>
